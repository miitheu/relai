...
export function useOpportunityEmailDraft(opportunity: any) {
  ...
  const generate = async (trigger: EmailDraftTrigger, userContext?: string, selectedContact?: any) => {
    ...
    const currentRequestId = ++requestIdRef.current; // Increment to invalidate stale responses
    setIsLoading(true);
    setEmailDraft(null);
    setVariants([]);
    setError(null);
    ...
    try {
      const bestContact = selectedContact || contacts.find((c: any) =>
        c.influence_level === 'Decision Maker'
      ) || contacts[0];

      // Match product fit to the opportunity's dataset, not just highest overall
      ...
      const topFit = productFits.find((pf: any) => pf.product_id === oppDatasetId)
        || productFits.find((pf: any) => oppProductIds.includes(pf.product_id))
        || productFits[0];

      const now = new Date();
      const daysInStage = opportunity.stage_entered_at
        ? differenceInDays(now, new Date(opportunity.stage_entered_at))
        : 0;
      const daysSinceActivity = opportunity.last_activity_at
        ? differenceInDays(now, new Date(opportunity.last_activity_at))
        : opportunity.updated_at
        ? differenceInDays(now, new Date(opportunity.updated_at))
        : 0;

      const products = (opportunity.opportunity_products || []).map(
        (p: any) => p.datasets?.name || p.name
      ).filter(Boolean);

      // Get full product details for the opportunity's dataset
      ...
      const { data, error } = await supabase.functions.invoke('campaign-email-draft', {
        body: {
          mode: 'opportunity_email',
          opportunity_name: opportunity.name,
          stage: opportunity.stage,
          trigger,
          client_name: opportunity.clients?.name || '',
          client_type: opportunity.clients?.client_type || '',
          dataset_name: opportunity.datasets?.name || null,
          products,
          days_in_stage: daysInStage,
          days_since_activity: daysSinceActivity,
          strategy_summary: summary?.strategy_summary || null,
          suggested_messaging: summary?.suggested_messaging || null,
          recommended_approach: summary?.recommended_approach || null,
          fit_score: topFit?.fit_score || null,
          evidence_summary: topFit?.evidence_summary || null,
          best_contact_name: bestContact?.name || null,
          best_contact_title: bestContact?.title || null,
          user_context: userContext || null,
          dataset_id: primaryDatasetId || null,
          dataset_description: primaryDataset?.description || null,
          dataset_coverage: primaryDataset?.coverage || null,
          dataset_use_cases: primaryDataset?.example_use_cases || null,
